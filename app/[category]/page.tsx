    ...
    <>
      ...
      <main className={`category-page category-${category}`}>
        ...
        <section className="category-feature shell">
          ...
          <div className="category-feature-copy"><span>ГЛАВНЫЙ МАТЕРИАЛ</span><h2>{feature.title}</h2><p>{feature.lead}</p><Link href={`/article/${feature.slug}`}>Читать →</Link></div>
        </section>
        <section className="section shell">
          <div className="mini-section-head"><h2>Новые материалы</h2><span>{items.length} материала в первой редакции</span></div>
        ...
        <section className="category-picks dark-section">
          <div className="shell"><div className="mini-section-head light-head"><h2>Выбор редакции</h2><span>Из других рубрик</span></div><div className="category-picks-grid">{recommendations.map(a => <StoryCard key={a.slug} article={a} />)}</div></div>
        </section>
      </main>
      <Footer />
    </>
